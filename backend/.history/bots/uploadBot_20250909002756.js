...
/cancel - Cancel current operation`);
        ...
        this.bot.command('uploadmovie', async (ctx) => {
            ...
            if (this.pendingOperations.has(userId) || this.seriesCreationState.has(userId) || this.editingState.has(userId)) {
        ...
        try {
            switch (state.step) {
                ...
                case 'awaiting_content_id':
                    await this.processContentId(ctx, userId, messageText);
                    break;
                    
                case 'awaiting_edit_field':
                    await this.processEditField(ctx, userId, messageText);
                    break;
                    
                case 'awaiting_edit_value':
                    await this.processEditValue(ctx, userId, messageText);
                    break;
                    
                case 'awaiting_delete_confirmation':
                    await this.processDeleteConfirmation(ctx, userId, messageText);
                    break;
    ...
    async processContentId(ctx, userId, contentId) {
        try {
    ...
    async processEditField(ctx, userId, fieldName) {
        const state = this.editingState.get(userId);
    ...
    async processEditValue(ctx, userId, newValue) {
        const state = this.editingState.get(userId);
    ...
    async processDeleteConfirmation(ctx, userId, confirmation) {
        const state = this.editingState.get(userId);

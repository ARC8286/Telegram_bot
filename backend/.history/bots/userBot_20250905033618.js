...
class UserBot {
    ...
    async handleContentRequest(ctx, contentId) {
        try {
            ...
            // If still not found, try searching by title as fallback
            if (!series) {
                const seriesFromTitle = await Series.findOne({
                    title: { $regex: new RegExp(contentId.replace(/_/g, ' '), 'i') }
                });
                
                if (seriesFromTitle) {
                    console.log('Found series by title match:', seriesFromTitle.title);
                    await this.showSeasonSelection(ctx, seriesFromTitle);
                    return;
                }
            }
            ...
            await ctx.reply('Sorry, the requested content was not found. This could be because:\n\n• The content was recently uploaded and may take a few minutes to appear\n• The deep link might be incorrect\n• The content may have been removed\n\nPlease try again or contact support if the issue persists.');

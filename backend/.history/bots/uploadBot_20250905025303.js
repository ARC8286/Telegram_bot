...
/cancel - Cancel current operation`);
        ...
        try {
            switch (state.step) {
                ...
                case 'awaiting_episode_number':
                    ...
                    await ctx.reply('Episode number saved! Now please provide a title for this episode (or type "skip" to use default):');
                    break;
                    
                case 'awaiting_episode_title':
                    if (messageText.toLowerCase() !== 'skip') {
                        state.data.tempEpisode.title = messageText;
                    } else {
                        state.data.tempEpisode.title = `Episode ${state.data.tempEpisode.episodeNumber}`;
                    }
                    
                    // Create episode
                    const season = await Season.findById(state.data.currentSeason);
                    const seriesForEpisode = await Series.findOne({ seriesId: state.data.seriesId });
                    ...
                    const newEpisode = new Episode({
                        ...
                        // telegramMessageId and telegramLink will be added after file upload
                    });
                    ...
                    // Store episode ID for file upload
                    state.data.tempEpisode.episodeId = newEpisode._id;
                    state.step = 'awaiting_episode_file_upload';
                    this.seriesCreationState.set(userId, state);
                    
                    await ctx.reply('✅ Episode created! Now please send the episode file:');
                    break;
    ...
    async handleSeriesFileUpload(ctx, userId) {
        ...
        if (state.step === 'awaiting_episode_file') {
            // First file upload for an episode
            const file = ctx.message.document || ctx.message.video;
            const fileId = file.file_id;
            const fileSize = file.file_size;
        ...
        } else if (state.step === 'awaiting_episode_file_upload') {
            // Final file upload for an episode
            const file = ctx.message.document || ctx.message.video;
            const fileId = file.file_id;
            const fileSize = file.file_size;
            ...
            try {
                // Get episode details
                const episode = await Episode.findById(state.data.tempEpisode.episodeId);
                const season = await Season.findById(episode.season);
                const series = await Series.findById(season.series);
                
                // Upload file to channel
                const caption = this.generateEpisodeCaption(series, season, episode);
                const isVideo = ctx.message.video !== undefined;
                ...
                episode.telegramMessageId = response.message_id.toString();
                episode.telegramLink = `https://t.me/c/${series.telegramChannel.replace('-100', '')}/${response.message_id}`;
                await episode.save();
    ...
    generateEpisodeCaption(series, season, episode) {
        let caption = `<b>${series.title}</b> - ${season.title}\n`;
        caption += `<b>Episode ${episode.episodeNumber}:</b> ${episode.title}\n`;

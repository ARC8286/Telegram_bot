...
class UserBot {
    ...
    async deliverMovie(ctx, movie) {
        try {
            ...
            // Create a clean info message (don't repeat what's already in the caption)
            let infoMessage = `🎬 <b>${movie.title}</b> (${movie.year})`;
            
            // Only add description if it's not already in the file caption
            if (movie.description) {
                infoMessage += `\n\n${movie.description}`;
            }
            
            // Only add genre if it's not already in the file caption
            if (movie.genre && movie.genre.length > 0) {
                infoMessage += `\n\n<b>Genre:</b> ${movie.genre.join(', ')}`;
            }
            
            // Send the clean info message
    ...
    async deliverEpisode(ctx, episode) {
        try {
            ...
            await ctx.reply(`📺 ${season.series.title} - ${episode.title}`);
    ...
    async sendAllEpisodes(ctx, series, season, episodes) {
        try {
            ...
            // Send each episode one by one (only episode number and title)
            for (const episode of episodes) {
                try {
                    ...
                    // Simple episode info: only episode number and title
                    await ctx.reply(`▶️ Episode ${episode.episodeNumber}: ${episode.title}`);

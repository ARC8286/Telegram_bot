...
class UserBot {
    ...
    setupHandlers() {
        ...
        this.bot.start(async (ctx) => {
            ...
            console.log('Deep link received:', contentId);
    ...
    async handleContentRequest(ctx, contentId) {
        try {
            ...
            // If not found by seriesId, try to find by contentId (for backward compatibility)
            const seriesByContentId = await Series.findOne({ contentId });
            if (seriesByContentId) {
                console.log('Found series by contentId:', seriesByContentId.title);
                await this.showSeasonSelection(ctx, seriesByContentId);
                return;
            }

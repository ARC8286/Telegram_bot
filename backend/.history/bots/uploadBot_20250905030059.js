        ...
        try {
            switch (state.step) {
                ...
                case 'awaiting_episode_title':
                    ...
                    // Store episode info for file upload
                    state.step = 'awaiting_episode_file_upload';
                    this.seriesCreationState.set(userId, state);
                    
                    await ctx.reply('✅ Episode details saved! Now please send the episode file:');
                    break;
    ...
    async handleSeriesFileUpload(ctx, userId) {
        ...
        if (state.step === 'awaiting_episode_file') {
            ...
            try {
                ...
                // Upload file to channel
                const caption = this.generateEpisodeCaption(series, season, state.data.tempEpisode);
                const isVideo = ctx.message.video !== undefined;

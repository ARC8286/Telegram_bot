...
<b>Support:</b> Contact @admin for permissions
        ...
        this.bot.on('text', async (ctx) => {
            ...
            } else if (this.viewState.has(userId)) {
                await this.handleViewOperations(ctx, userId, messageText);
            }
        });
...
<i>Last updated: ${new Date().toLocaleString()}</i>
    ...
    async handleCallbackQuery(ctx) {
        ...
        try {
            if (data.startsWith('view_movies_page_')) {
                ...
                const channel = data.replace('channel_', '');
                await this.handleChannelSelection(ctx, userId, channel);
            } else if (data === 'next_episode') {
                await this.handleNextEpisode(ctx, userId);
            } else if (data === 'new_season') {
                await this.handleNewSeason(ctx, userId);
            } else if (data === 'finish_series') {
                await this.handleFinishSeries(ctx, userId);
            } else if (data.startsWith('edit_')) {
                await this.handleEditFieldSelection(ctx, userId, data);
            } else if (data === 'confirm_delete') {
                await this.handleDeleteConfirmation(ctx, userId, true);
            } else if (data === 'cancel_delete') {
                await this.handleDeleteConfirmation(ctx, userId, false);
            } else if (data === 'cancel_edit') {
                this.editingState.delete(userId);
                await ctx.editMessageText('❌ Edit operation cancelled.');
                await ctx.answerCbQuery();
            } else {
    ...
    async handleChannelSelection(ctx, userId, channel) {
        try {
            if (this.pendingOperations.has(userId)) {
                const state = this.pendingOperations.get(userId);
                if (state.step === 'awaiting_channel') {
                    const channelId = TELEGRAM_CONFIG.CHANNELS[channel.toUpperCase()];
                    if (!channelId) {
                        await ctx.answerCbQuery('❌ Invalid channel selection');
                        return;
                    }
                    ...
                    await ctx.editMessageText('📁 Perfect! Now please send me the movie file:');
                    await ctx.answerCbQuery();
                }
            } else if (this.seriesCreationState.has(userId)) {
                const state = this.seriesCreationState.get(userId);
                if (state.step === 'awaiting_series_channel') {
                    const channelId = TELEGRAM_CONFIG.CHANNELS[channel.toUpperCase()];
                    if (!channelId) {
                        await ctx.answerCbQuery('❌ Invalid channel selection');
                        return;
                    ...
                    await ctx.editMessageText('✅ Series created successfully! Now let\'s add seasons.\n\n🏷️ Please send the season number (e.g., 1 for Season 1):');
                    await ctx.answerCbQuery();
                }
            }
        } catch (error) {
    ...
    async handleNextEpisode(ctx, userId) {
        try {
            if (this.seriesCreationState.has(userId)) {
                const state = this.seriesCreationState.get(userId);
                ...
                await ctx.editMessageText('📁 Great! Please send the next episode file:');
                await ctx.answerCbQuery();
            }
        } catch (error) {
    ...
    async handleNewSeason(ctx, userId) {
        try {
            if (this.seriesCreationState.has(userId)) {
                const state = this.seriesCreationState.get(userId);
                state.step = 'awaiting_season_number';
                ...
                await ctx.editMessageText('🏷️ Let\'s create a new season! Please send the season number:');
                await ctx.answerCbQuery();
            }
        } catch (error) {
            console.error('Error handling new season:', error);
            await ctx.answerCbQuery('❌ Error processing request');
        }
    }

    async handleFinishSeries(ctx, userId) {
        try {
            if (this.seriesCreationState.has(userId)) {
                const state = this.seriesCreationState.get(userId);
                ...
                await ctx.editMessageText(`✅ <b>Series completed successfully!</b>\n\n<b>Title:</b> ${series.title}\n<b>Seasons:</b> ${series.seasons.length}\n\n<b>Deep Link:</b> ${deepLink}\n\nUsers can now access this series using the deep link.`, { parse_mode: 'HTML' });
                await ctx.answerCbQuery();
                
                this.seriesCreationState.delete(userId);
            }
        } catch (error) {
            console.error('Error handling finish series:', error);
            await ctx.answerCbQuery('❌ Error completing series');
        }
    }

    async handleEditFieldSelection(ctx, userId, data) {
        try {
            if (this.editingState.has(userId)) {
                const state = this.editingState.get(userId);
                const field = data.replace('edit_', '');
                
                if (state.step === 'awaiting_edit_field') {
                    state.editField = field;
                    state.step = 'awaiting_edit_value';
                    this.editingState.set(userId, state);
                    
                    let currentValue = '';
                    if (field === 'genre' && state.content.genre) {
                        currentValue = `\n\n<b>Current value:</b> ${state.content.genre.join(', ')}`;
                    } else if (state.content[field]) {
                        currentValue = `\n\n<b>Current value:</b> ${state.content[field]}`;
                    }
                    
                    await ctx.editMessageText(`Please enter the new value for <b>${field}</b>:${currentValue}`, { parse_mode: 'HTML' });
                    await ctx.answerCbQuery();
                } else if (state.step === 'awaiting_season_field') {
                    state.editField = field;
                    state.step = 'awaiting_edit_value';
                    this.editingState.set(userId, state);
                    
                    let currentValue = '';
                    if (state.season[field]) {
                        currentValue = `\n\n<b>Current value:</b> ${state.season[field]}`;
                    }
                    
                    await ctx.editMessageText(`Please enter the new value for <b>${field}</b>:${currentValue}`, { parse_mode: 'HTML' });
                    await ctx.answerCbQuery();
                } else if (state.step === 'awaiting_episode_field') {
                    state.editField = field;
                    state.step = 'awaiting_edit_value';
                    this.editingState.set(userId, state);
                    
                    let currentValue = '';
                    if (state.episode[field]) {
                        currentValue = `\n\n<b>Current value:</b> ${state.episode[field]}`;
                    }
            ...
            console.error('Error handling edit field selection:', error);
            await ctx.answerCbQuery('❌ Error processing selection');
        }
    }

    async handleDeleteConfirmation(ctx, userId, confirm) {
        try {
            if (this.editingState.has(userId)) {
                const state = this.editingState.get(userId);
                ...
                if (state.contentType === 'movie') {
                    ...
                    await ctx.editMessageText('✅ Movie deleted successfully!');
                    ...
                    await ctx.editMessageText('✅ Series and all related content deleted successfully!');
                }
            ...
            await ctx.answerCbQuery('❌ Error deleting content');
        }
    }
...
`;
        ...
        try {
            switch (state.step) {
                ...
                case 'awaiting_episode_selection':
                    ...
                    state.step = 'awaiting_episode_field';
                    this.editingState.set(userId, state);
                ...
                case 'awaiting_edit_value':
                    ...
                    switch (state.editField) {
                        ...
                        case 'number':
                            const newNumber = parseInt(messageText);
                            if (isNaN(newNumber) || newNumber < 1) {
                                return ctx.reply('❌ Invalid episode number. Please enter a valid number:');
                            }
                            updateData.episodeNumber = newNumber;
                            break;
                    }
                    
                    await Episode.findByIdAndUpdate(state.episode._id, updateData);
                    await ctx.reply(`✅ Episode ${state.editField} updated successfully!`);
                    this.editingState.delete(userId);
                    break;
            }
        } catch (error) {
                    ...
                    const keyboard = {
                        inline_keyboard: [
                            [
                                ...
                                { text: '🔢 Number', callback_data: 'edit_season_number' }
                            ],
                            [
                                { text: '❌ Cancel', callback_data: 'cancel_edit' }
                            ]
                ...
                case 'awaiting_edit_value':
                    ...
                    switch (state.editField) {
                        ...
                        case 'number':
                            const newNumber = parseInt(messageText);
                            if (isNaN(newNumber) || newNumber < 1) {
                                return ctx.reply('❌ Invalid season number. Please enter a valid number:');
                            }
                            updateData.seasonNumber = newNumber;
                            break;
                    }
                    
                    await Season.findByIdAndUpdate(state.season._id, updateData);
                    await ctx.reply(`✅ Season ${state.editField} updated successfully!`);
                    this.editingState.delete(userId);
                    break;
            }
        } catch (error) {
    ...
    async handleViewOperations(ctx, userId, messageText) {
        // Handle view operations if needed
        // This can be expanded for more complex view interactions
    }
